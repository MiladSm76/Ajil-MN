    ...
    <nav className="h-[52px] w-full flex justify-center items-center">
      ...
      <Box className=" h-[100%] w-[85%] flex items-center border-b-[1px] border-[#D96B14] ">
          ...
          <ul className="flex gap-7 font-medium text-base text-[#757575] transition-all duration-500">
            ...
            >
              ...
              {flag && (
                <>
                  <Box
                    className={"fixed h-[450px] w-[85%]  translate-x-[-16.1%]"}
                  >
                    <Box
                      className={
                    ...
                    <Box
                      className={
                        "relative  bg-BackGroundN bg-cover bg-no-repeat shadow shadow-orange-200 w-[100%] rounded-[30px] h-[81%] mt-[10px] z-[50] flex p-0 m-0 "
                      }
                    >
                      <Box
                        className={
                      ...
                      >
                        ...
                        >
                          {arrProduct.map((e)=>{
                            return(
                              <li key={e.name} onMouseOver={()=>ShowList(e.product)} className="ProductLi ">{e.name}</li>  
                            )
                          })}
                        </ul>
                      </Box>
                      <Box className={"w-[23.7%]  h-[85%]  p-2 mt-7"}>
                        ...
                        product.map((item)=>{
                        return(
                          <li className="text-[14px] duration-500 transition-all headerNavbar">{item.name}</li>)
                        })}
                        </ul>
                      </Box>
                    </Box>
        ...
        <Box className={"flex items-center gap-8 ml-auto"}>
          ...
          <BiShoppingBag onClick={()=>setFlagDrawer(!flagDrawer)} className="text-[23px] headerNavbar text-[#757575]" />
        </Box>
      </Box>
    </nav>
  );
